...
### Intentionally Omitted (Time Prioritization)
1. **Unit tests** — Would add Jest tests for each scoring strategy and the weather aggregation logic. These are pure functions, highly testable.
2. **Caching layer** — Open-Meteo data could be cached (Redis/in-memory) with ~1hr TTL. Skipped for simplicity, but the service layer makes this a clean addition.
3. **Input validation** — Basic validation exists (lat/lon ranges, name length) but could be more robust with a validation library like Zod.
4. **Rate limiting** — No rate limiting on the GraphQL endpoint. In production, would add express-rate-limit.
5. **E2E tests** — Would use Playwright for critical path testing (search → select → view rankings).
6. **Accessibility audit** — Basic ARIA attributes added, but a full audit with screen readers is needed.
7. **Coastal proximity detection** — Surfing scores don't account for whether the location is near the coast. Would require a secondary geospatial check.

### Shortcuts Taken
1. **Scoring weights are heuristic** — Not calibrated against real activity preference data. A production system would use historical user feedback to tune weights.
2. **No persistent storage** — All data is fetched on-demand. Favorites, history, or saved rankings would require a database.
3. **Single-region deployment assumed** — No CDN, no edge caching. Would add these for global users.
4. **Snow depth may be 0 for many locations** — Open-Meteo doesn't always have snow depth data. The skiing scorer handles this gracefully but could integrate elevation data for better results.

### How I Would Fix Shortcuts
- **Scoring calibration**: Add an admin panel to adjust weights, or implement A/B testing with user feedback.
- **Coastal detection**: Use a reverse geocoding API or GIS dataset to determine proximity to coastline for surfing relevance.
- **Caching**: Add a `CacheService` that wraps the weather service with TTL-based caching. The service layer architecture makes this a single-file addition.
- **Testing**: Pure function scoring strategies are trivially testable — each gets a mock `dailyWeather` object and asserts score ranges.
